...
class getTreeFrame(Frame):
    ...
    def OnDoubleClick(self, event):
        ...
        if messagebox.askokcancel("Confirmation", "Proceed to copy password?", parent=self):
    ...
    def OnRightClick(self, event):
        ...
        if messagebox.askokcancel("Confirmation", "Proceed to display password?", parent=self):
            item = self.tree.focus()
            service = self.tree.item(item, "values")[0]
            var = self.data[service][1]
            var = encode.decode(var)
            messagebox.showinfo("Reveal Password", "Password: "+var, parent=self)
